...
class Vehicle(models.Model):
    ...
    def save(self, *args, **kwargs):
        # ✅ Auto-assign seats based on vehicle type
        if not self.seats:
...
class Booking(models.Model):
    ...
    security_deposit = models.PositiveIntegerField(default=2000)  # ✅ ADDED
    status = models.CharField(max_length=20, choices=STATUS, default="pending")
    ordered_at = models.DateTimeField(auto_now_add=True)

    def calculate_price(self):
        hours = (self.end_time - self.start_time).total_seconds() / 3600
        return round(hours * float(self.vehicle.price_per_hour), 2)
